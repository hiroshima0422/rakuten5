...
function favoritesRoomName() {
  // ナビゲーションバーのルーム表示を更新
              // ナビゲーションバーの情報を消去  143
            function clearNavbar() {
              
              // ナビゲーションバーのルーム表示を更新  453
...
// お気に入りを実際に表示する
function favoritesRoomName(roomName) {
  console.dir(roomName);
  var favoritesRoomName = "favorites";
  ...
  var roomRef = firebase.database().ref("favorites");
  // イベントハンドラを登録
  roomRef.on("value", function(childSnapshot, prevChildKey) {
...
function createMessageDiv(messageId, message) {
  ...
  $div.find(".glyphicon").on("click",function(){
      ...
      // favorites/UID/message-id/ 作成
//firebase.database().ref().child("favorites/" + currentUID + "/" + message.uid).push(new Date(message.time));      
//firebase.database().ref().child("favorites/" + currentUID + "/" + message.uid).push(new Date(message.time));      
//firebase.database().ref().child("favorites/" + currentUID + "/" + message.uid).push(message_data);                        
...
//2-1 「お気に入り」ルームの名前 を初期設定 
//var favoritesRoomName = ".favorites";

//2-2
// お気に入りルームを表示する
//function showFavorites() を作成

// ルームを実際に表示する
//function _showRoom{}のエラー処理まえに追記
//roomName が お気入りの場合 showFavorites()を実行するようにする。
