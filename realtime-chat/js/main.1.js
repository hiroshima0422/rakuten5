...
function createMessageDiv(messageId, message) {
  ...
  //$div.append($('<span>', {"class": "glyphicon glyphicon glyphicon-star"}));
...
$(document).ready(function() {
  ...
  $(".favorites__button").click(function() {
    ...
    // メッセージの取得
     // id属性をセット
  //$div.attr("id", "message-id-" + messageId);
     var uid = $(this).parent().attr('message-id');
    var text = $(this).parent().find(".message__text");
    var time = $(this).parent().find(".message__time");
...
firebase.database().ref().child("favorites/" + UID + "/" + message-id).push(favorites);
    
    // ルームと同様の場所に、「お気に入り一覧」へのリンクを表示
    ...
    // 　ユーザー毎のお気に入りメッセージをお気に入り追加順に表示
    
    // favoritesの表示用のdiv（jQueryオブジェクト）を作って返す
function createfavoritesDiv(messageId, message) {
  // HTML内のテンプレートからコピーを作成
  ...
  // 初期ルームの場合はルーム削除メニューを無効にする
  if (roomName === defaultRoomName) {
    $(".favorites-menuitem").addClass("disabled");
  } else {
    $(".favorites-menuitem").removeClass("disabled");
  }
    
    // glyphicon-starをクリックすると、お気に入りメッセージが解除され、glyphicon-star-emptyに戻して下さい。
...
// ルームを削除する
function deletefavorites(roomName) {
