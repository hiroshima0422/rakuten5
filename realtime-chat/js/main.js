...
console.log(currentUID);  
  ...
  // コールバックを登録
  favoritesRef.on("child_added", function(favoritesSnapshot) {
...
//console.log(Object.keys(favoritesSnapshot));
   var messageId = favoritesSnapshot.key;
    var favorite = favoritesSnapshot.val();
...
// お気に入りを実際に表示する
function showFavorites(roomName) {
  //console.dir(roomName);
  //var favoritesRoomName = "favorites";
  //var roomName = favoritesRoomName;
...
// ナビゲーションバーのルーム表示を更新
  ...
  $(".comment-form").hide();
  //$(".comment-form__submit").Attr("disabled");
...
// メッセ時を消す  
  ...
  // お気に入りメッセージ一覧をダウンロード、ユーザー毎のお気に入りメッセージをお気に入り追加順に表示
  //favorites/UID/message-id
  //firebase.database().ref("favorites/" + currentUID + "/" + message.uid).set({
  //var roomRef = firebase.database().ref("favorites/" +  currentUID);
  ...
  //        var v = data.val();   //データ取得
   //       var k = data.key; 
  favoritesRef.on("child_added", function(childSnapshot) {
    
    var messageId = childSnapshot.key;
    var favorite = childSnapshot.val();
    console.log(messageId);
    console.log(favorite);
    ...
    console.log(dbdata.favorites);
        ...
        //console.dir(childSnapshot.val().message);
        
        
      // お気に入りメッセージを表示
      addMessage(messageId, favorite.message);
...
/*************************************************************
   ...
   */
  //console.dir(dbdata);
   //console.dir(dbdata.favorites);
   //console.dir(dbdata.favorites[currentUID]);
   //console.dir(dbdata.favorites[currentUID][messageId]);
   /*
   if (dbdata.favorites) {
        
   
       if (dbdata.favorites[currentUID][messageId]) {
            $div.find(".message__favorite-link").addClass("glyphicon-star"); 
       }else{
            $div.find(".message__favorite-link").addClass("glyphicon-star-empty"); 
   ...
   */
   ...
   if(dbdata.favorites !== undefined){
    if(dbdata.favorites[messageId] !== undefined){
      //console.log(Object.keys(dbdata.favorites[messageId].createdAt));
  ...
  $div.find(".message__favorite-link").on("click",function(){
...
//***********************************************************
  ...
  // お気に入りボタンクリック
  if (roomName === "favorites") {
    console.log('_showRoom:' + roomName)
    showFavorites(roomName);
    return;
  }
...
function deleteRoom(roomName) {
  ...
  // TODO: ルームを削除
      firebase.database().ref("rooms/" + roomName).remove();  //6-8
  // TODO: ルーム内のメッセージも削除
      firebase.database().ref("messages/" + roomName).remove();   //6-8
...
//***********************************************************
  // お気に入りボタンクリック
  $("a.favorites").on("click",function(){
    if ( $(this).hasClass("chat_back") ){
      $(this).removeClass("chat_back").text("お気に入り");  
      ...
      $("#navbar").collapse("hide");
       ...
       $(this).addClass("chat_back").text("チャットルーム");
      roomName = favoritesRoomName;
      _showRoom(roomName);
    }
...
//***********************************************************
  ...
  $(".comment-form").submit(function() {
    ...
    firebase.database().ref().child("messages/" + currentRoomName).push(message);    //6.6
