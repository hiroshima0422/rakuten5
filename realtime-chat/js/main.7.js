...
// メッセ時を消す  
  ...
  roomRef.on("child_added", function(childSnapshot, prevChildKey) {
        ...
        //console.dir(childSnapshot.val()['ZShPXwxczRSucbOVOGFBNvFTbpl1']);
        //console.dir(childSnapshot.val()[currentUID]);
        //console.dir(childSnapshot.val()[currentUID][messageId]);
        //console.dir(childSnapshot.val()[currentUID][messageId]);
...
function createMessageDiv(messageId, message) {
  ...
  //console.dir(dbdata);
   ...
   //console.dir(dbdata.favorites[currentUID][message.uid]);
   if (dbdata.favorites !== null && dbdata.favorites[currentUID] !== null) {
     
      
      if (dbdata.favorites[currentUID][message.uid] !== null) {
            var favorite = dbdata.favorites[currentUID][message.uid];
            if (favorite) { // お気に入りが存在する場合
                  $div.find(".glyphicon").addClass("glyphicon-star");
            }else{
                  $div.find(".glyphicon").addClass("glyphicon-star-empty");
            }
  ...
  $div.find(".glyphicon-star-empty").on("click",function(){
      ...
      $div.find(".glyphicon-star").on("click",function(){
                    
                firebase.database().ref("favorites/" + currentUID + "/" + message.uid).remove();
